  ...
  void _initFromEnv() {
    ...
    if (gClient != null && gClient.isNotEmpty) {
      _providers['google'] = StaticOAuthProvider(
        ...
        redirectUrl: dotenv.env['GOOGLE_REDIRECT_URI'] ??
            'com.example.app:/oauth2redirect',
        scopes:
            _splitScopes(dotenv.env['GOOGLE_SCOPES'] ?? 'email,profile,openid'),
      );
    }
    // Microsoft
    ...
    if (mClient != null && mClient.isNotEmpty) {
      ...
      _providers['microsoft'] = StaticOAuthProvider(
        ...
        redirectUrl:
            dotenv.env['MS_REDIRECT_URI'] ?? 'msauth.com.example.app://auth',
        scopes: _splitScopes(
            dotenv.env['MS_SCOPES'] ?? 'openid,profile,email,offline_access'),
      );
    }
    ...
    if (dClient != null && dClient.isNotEmpty) {
      ...
      _providers['django'] = StaticOAuthProvider(
        clientId: dClient,
        authorizationEndpoint:
            dotenv.env['DJANGO_AUTH_URL'] ?? '$base/o/authorize/',
        tokenEndpoint: dotenv.env['DJANGO_TOKEN_URL'] ?? '$base/o/token/',
        redirectUrl: dotenv.env['DJANGO_REDIRECT_URI'] ??
            'com.example.app:/oauth2redirect',
